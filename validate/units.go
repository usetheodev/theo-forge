// Package validate provides validation and conversion functions
// for Kubernetes resource units (CPU, memory, storage).
package validate
...
func BinaryUnit(s string) error {
...
// DecimalUnit validates a decimal resource unit (CPU: m, k, M, G, T, P, E).
func DecimalUnit(s string) error {
...
func ResourceRequirements(r model.ResourceRequirements) error {
	// Validate CPU
	if r.Requests.CPU != "" {
		if err := DecimalUnit(r.Requests.CPU); err != nil {
		...
		if r.Limits.CPU != "" {
			if err := DecimalUnit(r.Limits.CPU); err != nil {
	...
	if r.Requests.Memory != "" {
		if err := BinaryUnit(r.Requests.Memory); err != nil {
		...
		if r.Limits.Memory != "" {
			if err := BinaryUnit(r.Limits.Memory); err != nil {
	...
	if r.Requests.EphemeralStorage != "" {
		if err := BinaryUnit(r.Requests.EphemeralStorage); err != nil {
			return fmt.Errorf("invalid ephemeral storage request: %w", err)
		}
		if r.Limits.EphemeralStorage != "" {
			if err := BinaryUnit(r.Limits.EphemeralStorage); err != nil {
